...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('sesli-hoşgeldin')
    .setDescription('Sesli Hoşgeldin Sistemi')
      ...
      option
        ...
        .addChannelOption(option =>
            option.setName('kanal')
              .setDescription('Ses Kanalını Ayarla')
              .setRequired(true)
  ...
  run: async (client, interaction) => {
    ...
    let abone = interaction.options.getRole('kayıtsız')
    if (islem === "sistem") {
      db.set(`seslihgsistem_${interaction.guild.id}`, true)
      db.set(`seslikanal_${interaction.guild.id}`, kanal.id)
      db.set(`seslihgyetkili_${interaction.guild.id}`, rol.id)
      db.set(`seslihgkayıtsız_${interaction.guild.id}`, abone.id)
      const ayarlandı = new EmbedBuilder()
        .setColor(0x00ff00)
        .setAuthor({ name: "Versty", iconURL: interaction.user.avatarURL() })
        .setDescription(`Sesli Hoşgeldin Sistemi Başarıyla Ayarlandı`)
        .addFields(
          { name: "Sesli Hoşgeldin Kanal", value: `${kanal}`, inline: true },
          { name: "Sesli Hoşgeldin Yetkili", value: `${rol}`, inline: true },
          { name: "Sesli Hoşgeldin Kayıtsız", value: `${abone}`, inline: true },
        )
        .setTimestamp()
        .setFooter({ text: "Versty", iconURL: "https://media.discordapp.net/attachments/944295908399538187/1138467462967464129/image.png" })
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(`
      Sesli Hoşgeldin Sıfırlamak İstediğine Eminmisin!
      `)
      ...
      const onay = new EmbedBuilder()
        ...
        .setAuthor({ name: "Sıfırlama İşlemi", iconURL: interaction.user.avatarURL() })
        .setDescription(`
        Sesli Hoşgeldin Sıfırlandı!
      `)
      const iptal = new EmbedBuilder()
        .setColor(0xff0000)
        .setAuthor({ name: "Sıfırlama İşlemi", iconURL: interaction.user.avatarURL() })
        .setDescription(`
        Sesli Hoşgeldin Sıfırlama İşlemi İptal Edildi!
      `)
      const collectorFilter = i => i.user.id === interaction.user.id;
      const confirmation = await interaction.channel.awaitMessageComponent({ filter: collectorFilter, time: 60000 });

      if (confirmation.customId === 'confirm') {
        db.delete(`seslihgsistem_${interaction.guild.id}`)
        db.delete(`seslikanal_${interaction.guild.id}`)
        db.delete(`seslihgyetkili_${interaction.guild.id}`)
        db.delete(`seslihgkayıtsız_${interaction.guild.id}`)
