...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription('Reklam Engel Sistemini Ayarla')
    ...
    .addChannelOption(option =>
      option.setName('log')
        .setDescription('Reklam Engel Log Kanalını Ayarla')
        .setRequired(true)
  ...
  run: async (client, interaction) => {
    ...
    if (islem === "sistem") {
      ...
      db.set(`reklamlog_${interaction.guild.id}`, kanal.id)
      const ayarlandı = new EmbedBuilder()
        .setColor(0x00ff00)
        .setAuthor({ name: "Versty", iconURL: interaction.user.avatarURL() })
        .setDescription(`Reklam Engel Sistemi Başarıyla Ayarlandı! Log Kanalı: ${kanal}`)
        .setTimestamp()
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(`
      Reklam Engel Sistemini Sıfırlamak İstediğine Eminmisin!
      `)
      ...
      const onay = new EmbedBuilder()
        ...
        .setAuthor({ name: "Sıfırlama İşlemi", iconURL: interaction.user.avatarURL() })
        .setDescription(`
      Reklam Engel Sistemi Sıfırlandı!
      `)
      const iptal = new EmbedBuilder()
        .setColor(0xff0000)
        .setAuthor({ name: "Sıfırlama İşlemi", iconURL: interaction.user.avatarURL() })
        .setDescription(`
      Reklam Engel Sistemini Sıfırlama İşlemi İptal Edildi!
      `)
      ...
      if (confirmation.customId === 'confirm') {
        ...
        db.delete(`reklamlog_${interaction.guild.id}`)
