...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription('Webhook Koruma Sistemini Ayarla')
    ...
    .addChannelOption(option =>
      option.setName('log')
        .setDescription('Webhook Koruma Log Kanalını Ayarla')
        .setRequired(true)
  ...
  run: async (client, interaction) => {
    ...
    if (islem === "sistem") {
      db.set(`webhooksistem_${interaction.guild.id}`, true)
      db.set(`webhooklog_${interaction.guild.id}`, kanal.id)
      const ayarlandı = new EmbedBuilder()
        .setColor(0x00ff00)
        .setAuthor({ name: "Versty", iconURL: interaction.user.avatarURL() })
        .setDescription(`Webhook Koruma Sistemi Başarıyla Ayarlandı! Log Kanalı: ${kanal}`)
        .setTimestamp()
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(`
      Webhook Koruma Sistemini Sıfırlamak İstediğine Eminmisin!
      `)
      ...
      const onay = new EmbedBuilder()
        ...
        .setAuthor({ name: "Sıfırlama İşlemi", iconURL: interaction.user.avatarURL() })
        .setDescription(`
      Webhook Koruma Sistemi Sıfırlandı!
      `)
      const iptal = new EmbedBuilder()
        .setColor(0xff0000)
        .setAuthor({ name: "Sıfırlama İşlemi", iconURL: interaction.user.avatarURL() })
        .setDescription(`
      Webhook Koruma Sistemini Sıfırlama İşlemi İptal Edildi!
      `)
      const collectorFilter = i => i.user.id === interaction.user.id;
      const confirmation = await interaction.channel.awaitMessageComponent({ filter: collectorFilter, time: 60000 });

      if (confirmation.customId === 'confirm') {
        db.delete(`webhooksistem_${interaction.guild.id}`)
        db.delete(`webhooklog_${interaction.guild.id}`)
